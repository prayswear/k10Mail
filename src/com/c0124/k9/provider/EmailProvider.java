...
import com.c0124.k9.cache.EmailProviderCacheCursor;
...
import com.c0124.k9.mail.store.LockableDatabase;
import com.c0124.k9.mail.store.UnavailableStorageException;
import com.c0124.k9.mail.store.LockableDatabase.DbCallback;
import com.c0124.k9.mail.store.LockableDatabase.WrappedException;
...
public class EmailProvider extends ContentProvider {
    ...
    public static final String AUTHORITY = "com.c0124.k9.provider.email";
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs,
        ...
        switch (match) {
            ...
            case MESSAGES_THREAD: {
                ...
                if (match == MESSAGES) {
                    //In our configuration, this is not used
                    cursor = getMessages(accountUuid, dbProjection, selection, selectionArgs,
                            sortOrder);
                } else if (match == MESSAGES_THREADED) {
                  //In our configuration, this is the view of a folder
                  //Using MessageViewFragment.THREADED_PROJECTION;
                  /*
                     SELECT m.id AS id,uid,a.internal_date AS internal_date,subject,a.date AS date,sender_list,to_list,cc_list,
                            a.read AS read,a.flagged AS flagged,a.answered AS answered,a.forwarded AS forwarded,
                            a.attachment_count AS attachment_count,folder_id,preview,root,name,mime_type,text_content,
                            thread_count 
                     FROM (
                            SELECT t.root AS thread_root,MAX(internal_date) AS internal_date,MAX(date) AS date,
                                   MIN(read) AS read,MAX(flagged) AS flagged,MIN(answered) AS answered,MIN(forwarded) AS forwarded,
                                   SUM(attachment_count) AS attachment_count,
                                   COUNT(t.root) AS thread_count 
                            FROM messages m LEFT JOIN threads t ON (t.message_id = m.id)LEFT JOIN folders f ON (m.folder_id = f.id) 
                            WHERE (deleted = 0 AND (empty IS NULL OR empty != 1)) AND (folder_id = ?) 
                            GROUP BY t.root
                          ) 
                    a LEFT JOIN threads t ON (t.root = a.thread_root) LEFT JOIN messages m ON (m.id = t.message_id) LEFT JOIN folders f ON (m.folder_id = f.id) 
                    WHERE m.date = a.date ORDER BY a.date DESC, id DESC
                  */
                    cursor = getThreadedMessages(accountUuid, dbProjection, selection,
                            selectionArgs, sortOrder);
                } else if (match == MESSAGES_THREAD) {
                    //Using MessageViewFragment.NON_THREADED_PROJECTION
                    //This is the view of a folder for messages in a same thread
                    /*
                      SELECT m.id AS id,uid,internal_date,subject,date,sender_list,to_list,cc_list,read,
                             flagged,answered,forwarded,attachment_count,folder_id,preview,root,name,mime_type,text_content 
                      FROM threads t JOIN messages m ON (m.id = t.message_id) LEFT JOIN folders f ON (m.folder_id = f.id) 
                      WHERE root = ? AND deleted = 0 AND (empty IS NULL OR empty != 1) 
