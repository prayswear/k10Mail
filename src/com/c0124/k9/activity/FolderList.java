...
import com.c0124.k9.Account.FolderMode;
import com.c0124.k9.activity.setup.AccountSettings;
import com.c0124.k9.activity.setup.AccountSetupBasics;
import com.c0124.k9.activity.setup.FolderSettings;
import com.c0124.k9.activity.setup.Prefs;
...
import com.c0124.k9.helper.SizeFormatter;
...
public class FolderList extends K9ListActivity {
    ...
    private static final String EXTRA_DIALOG_CONTENT = "dialog_content";
    ...
    AtomicBoolean needToShowDialog = new AtomicBoolean(false);
    ...
    * queueing up a remote update of the folder.
    ...
    public static Intent actionShowDialog(Context context, Account account, String title, String content) {
        Intent intent = new Intent(context, FolderList.class);
        intent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
        intent.putExtra(EXTRA_ACCOUNT, account.getUuid());
        intent.putExtra(EXTRA_DIALOG_TITLE, title);
        intent.putExtra(EXTRA_DIALOG_CONTENT, content);
        return intent;
    }
    
    public void showDialogForNotification() {
        if( !needToShowDialog.compareAndSet(true,false)) {
            return;
        }
        AlertDialog.Builder builder = new AlertDialog.Builder(this);
        builder.setMessage(mDialogContent).setTitle(mDialogTitle);
...
//        }
    ...
    public void onNewIntent(Intent intent) {
        ...
        if (intent.getExtras().get(EXTRA_DIALOG_TITLE) != null && intent.getExtras().get(EXTRA_DIALOG_CONTENT) != null) {
            mDialogTitle = (String) intent.getExtras().get(EXTRA_DIALOG_TITLE);
            mDialogContent = (String) intent.getExtras().get(EXTRA_DIALOG_CONTENT);
            needToShowDialog.set(true);
        }
    ...
    * like unread message count and read status are updated.
    ...
    @Override public void onResume() {
        ...
        ClientHelper.i("FolderList", "resuming the app");
        super.onResume();

        Account[] accounts = Preferences.getPreferences(this).getAccounts();
        if ((accounts.length < 1)  || (!mAccount.isAvailable(this))) {
                ...
                if (account.equals(mAccount)) {
                    ...
                    for (Folder folder : folders) {
                        try {
                            ...
                            if ( ( mAccount.getEncryptionFolderName().equalsIgnoreCase(folder.getName()) ||
                                mAccount.getDecryptionFolderName().equalsIgnoreCase(folder.getName())) && 
                                !com.c0124.utility.DebugPolicy.getInstance().toShowEncryptAndDecryptFolder()) {
                                continue;
                                //fMode = Folder.FolderClass.NO_CLASS;
                                //Jing: Originally set to NO_CLASS but this may mean there is no email in the folder
                                //and there seems not clearly how to use it, so just skip
                            }
